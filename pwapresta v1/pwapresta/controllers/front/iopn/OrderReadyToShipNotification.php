...
class OrderReadyToShipNotification {
	...
	public function update_order_detail($order_id , $data) {
			...
			else
			{
				...
				$this->acknowledge_order($order_id,$data);
			}
			...
			if($result[0]['current_state'] == 99)
			{
				$tablename = $prefix.'orders';
				$sql  = 'UPDATE `'.$tablename.'` set `current_state` = 2 where `id_order` = "'.$order_id.'" ';
				Db::getInstance()->Execute($sql);
				
				// Set the order status
				$history = new OrderHistory();
				$history->id_order = $order_id;
				$history->changeIdOrderState(2, $order_id, true);
	...
	public function update_cart_by_junglee_xml($order_id,$data){
	  ...
	  Db::getInstance()->Execute($sql);
		 ...
		 else
		 {
			...
			$address->postcode = (string)$xml->ProcessedOrder->ShippingAddress->PostalCode;
			$address->phone_mobile = (string)$xml->ProcessedOrder->ShippingAddress->PhoneNumber;
			$address->city = (string)$xml->ProcessedOrder->ShippingAddress->City.' '.(string)$xml->ProcessedOrder->ShippingAddress->State.' '.(string)$xml->ProcessedOrder->ShippingAddress->CountryCode;
			$address->active = 1;
			$address->add();
			$address_id = $address->id;
